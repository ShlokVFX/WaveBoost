...
class DistillationLossWithTopK(nn.Module):
    ...
    def forward(self, student_logits, teacher_logits, labels):
        ...
        if self.topk is not None and self.topk < V:
            ...
            topk_vals, topk_idx = torch.topk(flat_teacher, self.topk, dim=-1)

            # Mask all non-top-K positions with -inf
        ...
        soft_teacher = torch.softmax(teacher_logits_trunc / T, dim=-1)
        log_student = torch.log_softmax(student_logits_trunc / T, dim=-1)
        kl = self.kl_loss(log_student, soft_teacher) * (T * T)

        # ── Hard-label cross-entropy (on unscaled student logits) ────────────
